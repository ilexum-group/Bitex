...
func main() {
	...
	logger.Info("Starting Bitex analysis", map[string]string{"disk": *diskPath})

	analysis, err := disk.AnalyzeDisk(*diskPath)
	if err != nil {
		logger.Error("Bitex error", map[string]string{"error": err.Error()})
		os.Exit(2)
	}

	// Set case_id if provided
	if *caseID != "" {
	...
	if *serverURL != "" {
		...
		if err := sender.SendAnalysis(cfg, analysis); err != nil {
